...
import downloadTheFile from '@salesforce/apex/BoxController.downloadTheFile';
...
export default class Box extends LightningElement {
   ...
   // @track isFolder=true;
    ...
    deleteFile(event){
        ...
        downloadFile(event){
            var fId=event.target.dataset.idfile;
            console.log('file id is '+fId);
            downloadTheFile({ fileId: fId, accessToken: this.acToken})
                    .then(result => {
