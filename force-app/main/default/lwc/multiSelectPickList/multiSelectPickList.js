...
export default class MultiSelectPickList extends LightningElement {
    ...
    connectedCallback() {
            ...
            for(var i = 0; i < optionData.length; i++) {
                if(this.multiSelect) {
                    if(values.includes(optionData[i].value)) {
                        console.log('If condition of connectcallback is running');
        ...
        console.log('Value property in callback is '+values);
        this.value = value;
        this.values = values;
            ...
            for(var i = 0; i < options.length; i++) {
                if(options[i].value === selectedVal) {
                    if(this.multiSelect) {
                        ...
                        console.log('The value of this.values is '+this.values);
                        console.log('options value is '+options[i].value);
                        
                        if(this.values && this.values.includes(options[i].value)) {
                            console.log('Entered in splice part');
                         //   this.values.splice(this.values.indexOf(options[i].value), 1);
                        } else {
                            console.log('Entered in push part');
                            console.log('Data in option.value is '+options[i].value);
                        //    this.values.push(options[i].value);
                            console.log('Value of this.values in else part is '+this.values);
                        }
